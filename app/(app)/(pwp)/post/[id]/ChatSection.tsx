   ...
   return (
      <div className="flex gap-x-4 items-start w-full mt-6">
         ...
         <Button
            ...
            isIconOnly={true}
            variant="bordered"
            onClick={() => handleCreatePost().then(() => setLoading(false))}
            isLoading={loading}
            disabled={!newComment.text && !selectedMedia.length || loading}
         >
      ...
      <div className="w-full relative">
         {comments.map((comment) => (
            <div
               ...
               className={`py-4  ${level === 0 && "bottom-border pr-4 sm:pr-6"
                  } ${level > 0 && "pb-0"}`}
            >
               {comment.children && comment.children.length > 0 && (
                  <div
                     className={`absolute w-10 h-10 flex items-center justify-center ${level === 0 ? "left-4" : "-left-6"
                        }`}
                  >
                     <Button
                        isIconOnly={true}
